...
# def submission_should_compile(submission_result):
...
# def submission_should_evaluate(submission_result):
...
def submission_to_evaluate_on_testcase(submission_result, testcase_codename):
    ...
    """
    if not submission_should_evaluate(submission_result):
        return False
...
def submission_to_compile_operation(submission_result: SubmissionResult, submission: Submission, dataset: Dataset):
    """Generate the compile operation originating from a submission for a given
    dataset.
...
def submission_to_evaluate_operations(submission_result, submission, dataset):
    """Generate all evaluate operations originating from a submission for a
    given dataset.
...
class ESOperation(QueueItem):
    ...
    def is_compilation(self):
        ...
        """
        return self.type_ == ESOperation.COMPILATION or \
            self.type_ == ESOperation.USER_TEST_COMPILATION
